...
export const AddProductModal: React.FC<AddProductModalProps> = ({ visible, onClose, onSuccess, productToEdit }) => {
    ...
    const consumerKey = useAuthStore((state: any) => state.consumerKey);
    const consumerSecret = useAuthStore((state: any) => state.consumerSecret);

    // 🚀 استیت جادویی برای روان شدن انیمیشن باز شدن پنجره
    ...
    // 🚀 معماری جدید برای بارگذاری تنبل
    useEffect(() => {
        if (visible) {
            // اجازه می‌دهیم انیمیشن اسلایدِ پنجره در ۳۵۰ میلی‌ثانیه بدون لگ اجرا شود
    ...
    const fetchWordPressMedia = async (pageNum = 1) => {
        ...
        try {
            let cleanUrl = siteUrl.trim().replace(/\/+$/, ''); if (!cleanUrl.startsWith('http')) cleanUrl = `https://${cleanUrl}`;
            const res = await fetch(`${cleanUrl}/wp-json/wp/v2/media?per_page=40&page=${pageNum}`);
            const data = await res.json();
            if (Array.isArray(data)) { if (pageNum === 1) setWpMedia(data); else setWpMedia((prev) => [...prev, ...data]); setMediaPage(pageNum); }
        } catch (error) { if (pageNum === 1) Alert.alert('خطا', 'عدم ارتباط با رسانه وردپرس.'); } finally { setLoadingMedia(false); }
    };
    ...
    const uploadImageToWordPress = async (uri: string): Promise<any | null> => {
        try {
            ...
            const response = await fetch(`${cleanUrl}/wp-json/pishmo/v1/upload-image`, {
                method: 'POST',
            ...
            if (data.success && data.attachment_id) return { id: data.attachment_id };
            return null;
        } catch (e: any) {
            return null;
        }
        ...
        <Modal visible={visible} animationType="slide" transparent onRequestClose={() => { resetForm(); onClose(); }}>
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1, backgroundColor: 'rgba(15, 23, 42, 0.85)', justifyContent: 'flex-end' }}>
                ...
                <View style={{ backgroundColor: '#ffffff', borderTopLeftRadius: 32, borderTopRightRadius: 32, maxHeight: '88%', paddingTop: Platform.OS === 'android' ? StatusBar.currentHeight : 20, elevation: 15, flex: 1 }}>
                    ...
                    {/* 🚀 مسدود کردن رندر تا پایان انیمیشن تا صفحه درجا و بدون لگ باز شود */}
                    {!isModalReady ? (
                        ...
                        <ScrollView keyboardShouldPersistTaps="handled" nestedScrollEnabled={true} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingVertical: 16, paddingHorizontal: 20 }}>
                            ...
                            <View style={styles.submitContainer}>
                                <TouchableOpacity onPress={handleSubmit} disabled={loading} style={styles.submitBtnNarrow}>
                                    {loading ? <ActivityIndicator color="#ffffff" size="small" /> : <Text style={styles.submitBtnTxtNarrow}>{uploadingMedia ? 'آپلود تصاویر...' : 'تایید و ذخیره'}</Text>}
                                </TouchableOpacity>
