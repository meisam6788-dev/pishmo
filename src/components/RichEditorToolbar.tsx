...
import { createWooClient } from '../api/client'; // 🌟 اتصال به کلاینت امن ووکامرس
...
const RichEditorToolbar: React.FC<RichEditorProps> = ({ title, content, onChange, siteUrl }) => {
    ...
    // استیت برای نمایش لودینگ در زمان آپلود عکس از گوشی
    ...
    // 🌟 دریافت امنِ گالری از طریق تونل پیشمو (عبور از فایروال)
    const fetchMedia = async (pageNum = 1) => {
        ...
        try {
            ...
            if (Array.isArray(res.data)) {
                ...
                setHasMoreMedia(res.data.length === 40);
            } else {
    ...
    // 🌟 آپلود عکس لوکال به سایت قبل از قرار دادن در ادیتور
    const uploadAndInsertImage = async (uri: string) => {
        setIsUploadingImage(true);
        try {
            const manipulated = await (ImageManipulator as any).manipulateAsync(
                uri, [{ resize: { width: 1000, height: 1000 } }], { compress: 0.3, format: (ImageManipulator as any).SaveFormat.WEBP, base64: true }
            );

            const client = createWooClient();
            const res = await client.post('pishmo/upload-image', {
                image_base64: manipulated.base64,
                file_name: `editor-${Date.now()}.webp`
            });

            if (res.data && res.data.success && res.data.url) {
                richText.current?.insertImage(res.data.url);
            } else {
                Alert.alert('خطا', 'آپلود تصویر شکست خورد.');
            }
        } catch (e) {
            Alert.alert('خطا', 'ارتباط با سرور برای آپلود عکس برقرار نشد.');
        } finally {
    ...
    const pickLocalImage = async () => {
        ...
        const result = await ImagePicker.launchImageLibraryAsync({ 
            mediaTypes: 'images' as any, // 🌟 رفع اخطار زرد اکسپو
            quality: 1 
        });
        if (!result.canceled && result.assets.length > 0) { 
            uploadAndInsertImage(result.assets[0].uri); 
        }
    };
                ...
                <View style={{ flex: 1, backgroundColor: '#f8fafc', paddingTop: Platform.OS === 'android' ? StatusBar.currentHeight : 40 }}>
                    ...
                    <View style={styles.fullScreenHeader}>
                        ...
                        <View style={{ flexDirection: 'row-reverse', alignItems: 'center' }}>
                            ...
                            {isUploadingImage && <ActivityIndicator color="#3b82f6" size="small" style={{ marginRight: 8 }} />}
                        </View>
