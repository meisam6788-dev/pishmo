...
import {
    ...
    RefreshControl, Alert, ScrollView, Platform, Linking, Modal, TextInput, KeyboardAvoidingView
} from 'react-native';
...
export const CustomerClubScreen: React.FC = () => {
    ...
    const fetchClubData = async (pageNum = 1, isRefresh = false) => {
        ...
        try {
            ...
            const response = await client.get('pishmo/v1/crm', {
                params: {
                    page: pageNum, per_page: 50, segment: segmentParam,
    ...
    const handleRefresh = useCallback(() => { setRefreshing(true); setPage(1); fetchClubData(1, true); }, [activeSegments, fMinOrders, fMaxOrders, fMinSpent, fMaxSpent, fDaysFrom, fDaysTo, selectedCategories, catMode]);
    ...
    // 🚀 خروجی واقعی اکسل (ضدگلوله شده با Any برای دور زدن تایپ‌اسکریپت)
    const exportDataToFile = async (specificIds?: number[]) => {
        try {
            ...
            const response = await client.get('pishmo/v1/crm', {
                params: {
                    return_type: 'export',
                    include_ids: specificIds ? specificIds.join(',') : '',
            ...
            // 🚀 جادوی دور زدن ارورهای TS: تبدیل FileSystem و Sharing به Any
            const FS: any = FileSystem;
            const ShareModule: any = Sharing;

            const dirPath = FS.documentDirectory || FS.cacheDirectory;
            if (!dirPath) {
                Alert.alert('خطا', 'حافظه موقت یا اصلی برنامه در دسترس نیست.');
                setLoading(false);
                return;
            }

            const fileUri = dirPath + `Pishmo_Customers_${Date.now()}.csv`;

            await FS.writeAsStringAsync(fileUri, csvContent, { encoding: FS.EncodingType?.UTF8 || 'utf8' });

            const isAvailable = await ShareModule.isAvailableAsync();
            if (isAvailable) {
                await ShareModule.shareAsync(fileUri, {
                    mimeType: 'text/csv',
                    dialogTitle: 'ذخیره فایل در گوشی',
            ...
            Alert.alert('خطای ذخیره‌سازی', error?.message || 'مشکلی در ساخت فایل پیش آمد.');
        } finally {
            setLoading(false);
        }
    };
            ...
            { text: `لود شده‌ها (${customers.length})`, onPress: () => setSelectedIds(customers.map(c => c.id)) },
            {
                text: `انتخاب همه (${totalServerCount})`, onPress: async () => {
                    ...
                    try {
                        ...
                        const catNotIn = catMode === 'not_in' ? selectedCategories.join(',') : '';
                        const response = await client.get('pishmo/v1/crm', {
                            params: { return_type: 'ids', segment: segmentParam, min_orders: fMinOrders, max_orders: fMaxOrders, min_spent: fMinSpent, max_spent: fMaxSpent, days_from: fDaysFrom, days_to: fDaysTo, cat_in: catIn, cat_not_in: catNotIn }
                        });
        ...
        try {
            ...
            if (couponCodeInput.trim() && couponAmountInput.trim()) {
                ...
                const payload: any = {
                    ...
                    amount: couponAmountInput.trim(),
                    description: `ساخته شده اختصاصی برای گروه: ${groupNameInput.trim()}`
                };
                ...
                if (couponDaysInput.trim()) {
                    ...
                    if (!isNaN(days)) {
                        ...
                        payload.date_expires = futureDate.toISOString();
                    }
                }

                if (couponMinAmountInput.trim()) payload.minimum_amount = couponMinAmountInput.trim();

                if (couponMaxDiscountInput.trim()) {
                    payload.meta_data = [
                        { key: '_wc_max_discount', value: couponMaxDiscountInput.trim() },
                        { key: 'maximum_amount', value: couponMaxDiscountInput.trim() }
                    ];
                }

                try {
                    await client.post('coupons', payload);
                    createdCouponMsg = `\n🎁 کد تخفیف "${couponCodeInput}" در سایت ساخته شد.`;
                } catch (apiError: any) {
                    if (apiError.response?.data?.code === 'woocommerce_rest_coupon_code_already_exists') {
                        Alert.alert('خطای ساخت کوپن', 'کد تخفیفی که وارد کردید از قبل وجود دارد! لطفاً یک کد جدید و غیرتکراری بنویسید.');
                        setIsSavingGroup(false); return;
                    } else {
                        Alert.alert('هشدار اتصال', 'خطا در ارتباط با سرور برای ساخت کوپن. اما اطلاعات گروه در گوشی شما ذخیره می‌شود.');
                    }
                }
            }
            ...
            const groupData: SavedGroup = {
                ...
                couponCode: couponCodeInput.trim(), couponAmount: couponAmountInput.trim(),
                couponType: couponType, couponDays: couponDaysInput.trim(),
                couponMinAmount: couponMinAmountInput.trim(), couponMaxDiscount: couponMaxDiscountInput.trim()
            };

            if (editingGroupId) {
                updatedGroups = updatedGroups.map(g => g.id === editingGroupId ? groupData : g);
            } else {
            ...
            Alert.alert('عملیات موفق', `گروه "${groupNameInput.trim()}" ذخیره شد.${createdCouponMsg}`);
            setIsSelectMode(false); setSelectedIds([]); setGroupModalVisible(false);
        } catch (error: any) {
            Alert.alert('خطا', 'مشکلی در ذخیره اطلاعات گروه پیش آمد.');
        } finally {
    ...
    const deleteGroup = (id: string) => {
        Alert.alert('حذف گروه', 'آیا از حذف این گروه مطمئن هستید؟ (کوپن‌های ساخته شده در سایت حذف نمی‌شوند)', [
            { text: 'انصراف', style: 'cancel' },
            {
                text: 'حذف', style: 'destructive', onPress: async () => {
                    const updated = savedGroups.filter(g => g.id !== id);
                    setSavedGroups(updated);
                    await AsyncStorage.setItem('@pishmo_customer_groups', JSON.stringify(updated));
                }
            }
        ]);
    };
            ...
            >
                <View style={styles.cardTopRow}>
                    ...
                    <View style={[styles.badgePill, { backgroundColor: item.badgeBg }]}>
                        <Text style={[styles.badgeTxt, { color: item.badgeColor }]}>{item.segmentTitle}</Text>
                    </View>
                </View>
                            ...
                            <View key={group.id || index.toString()} style={[styles.groupCard, { backgroundColor: theme.card, borderColor: theme.border }]}>
                                ...
                                <View style={{ flexDirection: 'row-reverse', gap: 10 }}>
                                    <TouchableOpacity style={styles.groupActionBtn} onPress={() => {
                                        const groupPhones = customers.filter(c => group.customerIds.includes(c.id)).map(c => c.phone);
                                        openPatternModal(groupPhones);
                                    }}>
                                        <Feather name="mail" size={14} color="#fff" />
                                        <Text style={styles.groupActionTxt}>پترن پیامک</Text>
                                    </TouchableOpacity>
                                    <TouchableOpacity style={[styles.groupActionBtn, { backgroundColor: '#10b981' }]} onPress={() => {
                                        const groupPhones = customers.filter(c => group.customerIds.includes(c.id)).map(c => c.phone);
                                        handleBulkSMS(groupPhones);
                                    }}>
                ...
                <>
                    <View style={[styles.topBar, { backgroundColor: theme.card, borderBottomColor: theme.border }]}>
                        ...
                        <View style={{ flexDirection: 'row-reverse', alignItems: 'center' }}>
                            {!isSelectMode ? (
                                <>
                                    <TouchableOpacity style={[styles.iconBtn, { backgroundColor: theme.input, borderColor: theme.border, borderWidth: 1 }]} onPress={() => exportDataToFile()}>
                                        <Feather name="download" size={16} color="#3b82f6" />
                                    </TouchableOpacity>
                                    <TouchableOpacity style={[styles.iconBtn, { backgroundColor: theme.input, borderColor: theme.border, borderWidth: 1, marginHorizontal: 8 }]} onPress={() => setIsSelectMode(true)}>
                                        <Feather name="check-square" size={16} color={theme.textMuted} />
                                    </TouchableOpacity>
                                    <TouchableOpacity style={[styles.iconBtn, { backgroundColor: theme.input, borderColor: theme.border, borderWidth: 1, position: 'relative' }]} onPress={() => setFilterModalVisible(true)}>
                                        <Feather name="filter" size={16} color="#10b981" />
                                        {hasActiveFilters && <View style={styles.filterDot} />}
                                    </TouchableOpacity>
                                </>
                            ) : (
                                <TouchableOpacity style={[styles.iconBtn, { backgroundColor: '#fee2e2', borderColor: '#fca5a5', borderWidth: 1 }]} onPress={() => { setIsSelectMode(false); setSelectedIds([]); }}>
                                    <Text style={{ color: '#ef4444', fontWeight: 'bold', fontSize: 11 }}>لغو انتخاب</Text>
                                </TouchableOpacity>
                            )}
                        </View>
                    </View>
                    ...
                    {loading && page === 1 ? (
                        <View style={styles.centerContainer}><ActivityIndicator size="large" color={theme.primary} /><Text style={[styles.loadingTxt, { color: theme.textMuted }]}>در حال پردازش داده‌های سرور...</Text></View>
                    ) : customers.length === 0 ? (
                        <View style={styles.centerContainer}>
                            ...
                            <TouchableOpacity style={{ marginTop: 15 }} onPress={clearFilters}><Text style={{ color: '#10b981', fontWeight: 'bold' }}>حذف فیلترها</Text></TouchableOpacity>
                        </View>
                    ) : (
                        <FlatList
                            data={customers}
                        ...
                        <View style={[styles.bulkActionBar, { backgroundColor: theme.card, borderTopColor: theme.border }]}>
                            <View style={{ flexDirection: 'row-reverse', alignItems: 'center' }}>
                                <TouchableOpacity style={styles.selectAllBtn} onPress={handleSelectAll}>
                                    <Text style={styles.selectAllBtnTxt}>{selectedIds.length > 0 ? 'حذف همه' : 'انتخاب همه'}</Text>
                                </TouchableOpacity>
                                <Text style={{ color: theme.textMuted, fontSize: 11, fontWeight: 'bold', marginRight: 10 }}>{selectedIds.length} مورد</Text>
                            </View>
                            <View style={{ flexDirection: 'row-reverse' }}>
                                <TouchableOpacity style={[styles.bulkActionBtn, { backgroundColor: '#f59e0b' }]} onPress={() => openGroupModal()}>
                                    <Feather name="folder-plus" size={16} color="#fff" />
                                </TouchableOpacity>
                                <TouchableOpacity style={[styles.bulkActionBtn, { backgroundColor: '#10b981', marginHorizontal: 6 }]} onPress={() => openPatternModal(customers.filter(c => selectedIds.includes(c.id)).map(c => c.phone))}>
                                    <Feather name="mail" size={16} color="#fff" />
                                </TouchableOpacity>
                                <TouchableOpacity style={[styles.bulkActionBtn, { backgroundColor: '#3b82f6' }]} onPress={() => exportDataToFile(selectedIds)}>
                ...
                <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.centerModalOverlay}>
                    <View style={[styles.centerModalContainer, { backgroundColor: theme.card, maxHeight: '90%' }]}>
                        ...
                        <ScrollView showsVerticalScrollIndicator={false}>
                            ...
                            <Text style={{ fontSize: 10, color: theme.textMuted, textAlign: 'right', marginBottom: 10 }}>با پر کردن فرم زیر، یک کوپن واقعی در سایت ساخته می‌شود.</Text>
                            ...
                            <View style={{ flexDirection: 'row-reverse', alignItems: 'center', justifyContent: 'space-between', marginTop: 15 }}>
                                <View style={{ flex: 1, marginLeft: 5 }}>
                                    <Text style={[styles.filterGroupTitle, { color: theme.text, fontSize: 11, marginBottom: 4 }]}>حداقل خرید (تومان)</Text>
                                ...
                                <View style={{ flex: 1, marginRight: 5 }}>
                                    <Text style={[styles.filterGroupTitle, { color: theme.text, fontSize: 11, marginBottom: 4 }]}>سقف تخفیف درصدی</Text>
                            ...
                            <TouchableOpacity
                                style={[styles.btnApply, { marginTop: 25, marginBottom: 10, opacity: isSavingGroup ? 0.7 : 1 }]}
            ...
            <Modal visible={patternModalVisible} animationType="fade" transparent onRequestClose={() => setPatternModalVisible(false)}>
                <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.centerModalOverlay}>
                    <View style={[styles.centerModalContainer, { backgroundColor: theme.card }]}>
                        <View style={[styles.modalHeader, { borderBottomColor: theme.border }]}>
                            <Text style={[styles.modalTitle, { color: theme.text }]}>ارسال پیامک پترن‌دار (الگو)</Text>
                            <TouchableOpacity onPress={() => setPatternModalVisible(false)}><Feather name="x" size={24} color={theme.textMuted} /></TouchableOpacity>
                        </View>
                        <Text style={{ textAlign: 'right', fontSize: 12, color: theme.textMuted, marginBottom: 15 }}>تعداد گیرندگان معتبر: {targetPhonesForPattern.length} نفر</Text>

                        <Text style={[styles.filterGroupTitle, { color: theme.text }]}>شناسه پترن (کد الگو)</Text>
                        ...
                        <Text style={[styles.filterGroupTitle, { color: theme.text }]}>متغیرها (با کاما جدا کنید)</Text>
                        ...
                        <TouchableOpacity style={[styles.btnApply, { marginTop: 25, backgroundColor: '#3b82f6' }]} onPress={sendPatternSMS}>
                            <Text style={styles.btnApplyTxt}>ثبت در صف ارسال</Text>
                        </TouchableOpacity>
                    </View>
                </KeyboardAvoidingView>
            </Modal>
                ...
                <View style={styles.drawerOverlay}>
                    ...
                    <View style={[styles.drawerContainer, { backgroundColor: theme.card }]}>
                        ...
                        <ScrollView showsVerticalScrollIndicator={false} style={{ flex: 1 }}>
                            <Text style={[styles.filterGroupTitle, { color: theme.text }]}>وضعیت وفاداری (امکان انتخاب چندگانه)</Text>
...
const styles = StyleSheet.create({
    ...
    bulkActionBar: { position: 'absolute', bottom: 85, left: 16, right: 16, borderRadius: 16, flexDirection: 'row-reverse', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 12, elevation: 5, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 5 },
    selectAllBtn: { paddingHorizontal: 16, paddingVertical: 10, borderRadius: 10, backgroundColor: '#3b82f6', elevation: 2 },
    selectAllBtnTxt: { color: '#ffffff', fontSize: 12, fontWeight: '900' },
    bulkActionBtn: { width: 44, height: 44, borderRadius: 12, justifyContent: 'center', alignItems: 'center' },
