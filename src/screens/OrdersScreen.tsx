...
import {
    ...
    Alert, RefreshControl, Modal, ScrollView, TextInput, Image, Linking, Share, Platform, Animated, PanResponder, InteractionManager
} from 'react-native';
            ...
            <Animated.View style={{ transform: [{ translateX: pan }] }} {...panResponder.panHandlers}>
                <TouchableOpacity style={[styles.orderCard, isSelected && { borderColor: '#3b82f6', borderWidth: 2 }]} onPress={onOpenModal} onLongPress={onLongPress} activeOpacity={0.9}>
                    <View style={styles.cardRow}>
                        ...
                        <View style={{ flexDirection: 'row', alignItems: 'center' }}><View style={styles.itemsCountBadge}><Text style={styles.itemsCountTxt}>{itemsCount} کالا</Text></View><Text style={[styles.orderIdTxt, { marginLeft: 8 }]}>#{item.id}</Text></View>
                    </View>
                    <View style={[styles.cardRow, { marginBottom: 8 }]}><View style={[styles.statusBadge, { backgroundColor: badge.bg }]}><Text style={[styles.statusTxt, { color: badge.color }]}>{badge.label}</Text></View><Text style={styles.orderDateTxt}>{toShamsi(item.date_created)}</Text></View>
                    <View style={styles.cardFooter}><Text style={styles.totalPriceTxt}>{formatPrice(item.total)} تومان</Text>{getShippingBadge(item)}</View>
...
export const OrdersScreen: React.FC = () => {
    ...
    const fetchOrders = useCallback(async (pageNum = 1, isRefresh = false, searchStr = globalSearchQuery) => {
        ...
        try {
            ...
            if (query !== '') { query = query.replace('#', ''); params.search = query; }
    ...
    }, [statusFilter, globalSearchQuery]);

    useEffect(() => { setPage(1); setSelectedOrders([]); fetchOrders(1); }, [fetchOrders]);
    ...
    const handleGlobalSearch = (text: string) => {
        ...
        globalSearchTimeout.current = setTimeout(() => fetchOrders(1, false, text), 800);
    };
    ...
    const handleLoadMore = () => { 
        if (!loading && !loadingMore && hasMore && !isSilentUpdating) { 
            ...
            setPage(nextPage); 
            fetchOrders(nextPage, false, globalSearchQuery); 
        } 
    };
    ...
    const toggleOrderSelection = (id: number) => {
        setSelectedOrders(prev => { if (prev.includes(id)) return prev.filter(oId => oId !== id); return [...prev, id]; });
    };
    ...
    const handleAction = (url: string, errorMsg: string) => { Linking.canOpenURL(url).then(supported => { if (supported) Linking.openURL(url); else Alert.alert('خطا', errorMsg); }); };
    ...
    const handleOpenModal = (order: any) => {
        ...
        client.get('orders', { params: { ...searchParam, per_page: 100 } }).then(res => {
            const count = res.data.length;
            const totalSpent = res.data.reduce((sum: number, o: any) => sum + parseFloat(o.total || '0'), 0);
            setCustomerStats({ loading: false, count: count > 0 ? count : 1, total: totalSpent > 0 ? totalSpent : order.total, isGuest: order.customer_id === 0 });
        }).catch(() => { setCustomerStats({ loading: false, count: 1, total: order.total, isGuest: order.customer_id === 0 }); });
    };
                    ...
                    <View style={styles.modalContainer}>
                        ...
                        {selectedOrder && (
                            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.modalBody}>
                                    ...
                                    {statusAccordionOpen && (
                                        <View style={styles.accordionContent}>
                                            <View style={styles.statusActionBox}>
                                                {[{ id: 'processing', label: 'در حال انجام' }, { id: 'completed', label: 'تکمیل شده' }, { id: 'pending', label: 'در انتظار پرداخت' }, { id: 'cancelled', label: 'لغو شده' }].map((st) => {
                                ...
                                <Text style={styles.sectionMainTitle}>🛒 اقلام خریداری شده:</Text>
                                {selectedOrder.line_items?.map((item: any, idx: number) => {
                                    const qtyNum = parseInt(item.quantity) || 1;
                                    return (
                                        <View key={idx} style={styles.singleProductCard}>
                                ...
                                {selectedOrder.customer_note ? (
                                    <View style={styles.noteCard}>
                                        <View style={{ flexDirection: 'row-reverse', alignItems: 'center', marginBottom: 6 }}><Feather name="message-square" size={16} color="#ca8a04" /><Text style={styles.noteTitle}>یادداشت مشتری:</Text></View>
                                        <Text style={styles.noteTxt}>{selectedOrder.customer_note}</Text>
                                    </View>
                                ) : null}
                                                ...
                                                <>
                                                    ...
                                                    {selectedOrder.billing?.phone && (
                                                        <View style={styles.actionsBar}>
                                                            <TouchableOpacity style={[styles.actionIconBtn, { backgroundColor: '#dcfce7' }]} onPress={() => handleAction(`tel:${selectedOrder.billing.phone}`, 'تماس برقرار نشد')}><Text style={[styles.actionIconTxt, { color: '#16a34a' }]}>تماس</Text></TouchableOpacity>
                                                            <TouchableOpacity style={[styles.actionIconBtn, { backgroundColor: '#e0f2fe' }]} onPress={() => handleAction(`sms:${selectedOrder.billing.phone}`, 'پیامک باز نشد')}><Text style={[styles.actionIconTxt, { color: '#0284c7' }]}>پیامک</Text></TouchableOpacity>
                                                            <TouchableOpacity style={[styles.actionIconBtn, { backgroundColor: '#dbeafe' }]} onPress={() => handleAction(`https://t.me/+98${selectedOrder.billing.phone.replace(/^0/, '')}`, 'تلگرام باز نشد')}><Text style={[styles.actionIconTxt, { color: '#2563eb' }]}>تلگرام</Text></TouchableOpacity>
                                                        </View>
                                                    )}
                                ...
                                <View style={styles.customerStatsBox}>
                                    {customerStats.loading ? <ActivityIndicator size="small" color="#3b82f6" /> : (
                                        <>
                                            <View style={{ alignItems: 'center', flex: 1 }}><Text style={styles.customerStatsLabel}>مجموع خرید مشتری</Text><Text style={styles.customerStatsValue}>{formatPrice(customerStats.total)} تومان</Text></View>
                                            <View style={{ height: '100%', width: 1, backgroundColor: '#cbd5e1' }} />
                                            <View style={{ alignItems: 'center', flex: 1 }}><Text style={styles.customerStatsLabel}>تعداد سفارشات</Text><Text style={styles.customerStatsValue}>{customerStats.isGuest ? 'مهمان' : `${customerStats.count} بار`}</Text></View>
                                        </>
                                    )}
                                </View>
                        ...
                        <ViewShot ref={invoiceViewRef} options={{ format: "jpg", quality: 0.9 }}>
                            <View style={styles.invoicePaper}>
                                <View style={styles.invoiceModernHeader}>
                                    <View style={styles.invoiceLogoPlaceholder}><Text style={styles.invoiceLogoTxt}>L</Text></View>
                                    <View style={{ alignItems: 'flex-end' }}>
                                        <Text style={styles.invoiceStoreName}>فروشگاه لبسی</Text>
                                        <Text style={styles.invoiceStoreDomain}>www.lebsi.ir</Text>
                                    </View>
                                </View>
                                ...
                                <Text style={styles.invoiceFooterMsg}>با تشکر از خرید شما از لبسی 💜</Text>
                            </View>
                        </ViewShot>
                    </ScrollView>
                </View>
...
const styles = StyleSheet.create({
    ...
    orderCard: { backgroundColor: '#ffffff', borderRadius: 12, padding: 14, marginBottom: 0, borderWidth: 1, borderColor: '#e2e8f0', elevation: 1 },
    cardRow: { flexDirection: 'row-reverse', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 },
    customerNameTxt: { fontSize: 14, fontWeight: '900', color: '#0f172a', textAlign: 'right', flex: 1 },
    ...
    cardFooter: { flexDirection: 'row-reverse', justifyContent: 'space-between', alignItems: 'center', borderTopWidth: 1, borderTopColor: '#f1f5f9', paddingTop: 10, marginTop: 4 },
    ...
    customerStatsBox: { flexDirection: 'row-reverse', backgroundColor: '#f0f9ff', borderWidth: 1, borderColor: '#bae6fd', borderRadius: 12, padding: 12, marginBottom: 12, marginTop: 6 },
    ...
    noteCard: { backgroundColor: '#fefce8', borderWidth: 1, borderColor: '#fef08a', borderRadius: 12, padding: 12, marginBottom: 12 },
    noteTitle: { fontSize: 12, fontWeight: '900', color: '#a16207', textAlign: 'right', marginRight: 4 },
    noteTxt: { fontSize: 12, color: '#854d0e', textAlign: 'right', lineHeight: 20 },
    ...
    singleProductCard: { backgroundColor: '#ffffff', borderWidth: 1, borderColor: '#cbd5e1', borderRadius: 12, padding: 12, marginBottom: 12, elevation: 1 },
