...
import {
    ...
    RefreshControl, Dimensions, Modal, TextInput, Switch, Platform, Alert, Linking, Image, FlatList
} from 'react-native';
...
const DashboardScreen: React.FC = () => {
    ...
    const switchSite = useAuthStore(state => state.switchSite);
    ...
    const [siteSwitcherVisible, setSiteSwitcherVisible] = useState(false); 
    ...
    const fetchData = async (filter: DateFilter, isRefresh = false) => {
        ...
        try {
            ...
            const labels = daysTemplate.map((d, index) => {
                ...
                if (filter === 'today') step = Math.ceil(totalDays / 5);
                if (index === 0 || index === totalDays - 1 || index % step === 0) return d.jalaliDay;
                return ' ';
            });
    ...
    // 🌟 باز کردن لینک‌های هاردکد شده بدون بررسی اولیه (حل قطعی ارور نصب نیست)
    const openSupportLink = async (url: string) => {
        try {
            await Linking.openURL(url);
        } catch (error) {
            Alert.alert('خطا', 'امکان باز کردن این لینک وجود ندارد. احتمالاً برنامه مربوطه نصب نیست.');
        }
    };
        ...
        <View style={[styles.container, { backgroundColor: theme.bg }]}>
            <View style={[styles.header, { backgroundColor: theme.card, borderBottomColor: theme.border }]}>
                <View style={{ alignItems: 'flex-end', flex: 1 }}>
                    <TouchableOpacity 
                        style={{ flexDirection: 'row-reverse', alignItems: 'center', backgroundColor: theme.input, paddingVertical: 6, paddingHorizontal: 12, borderRadius: 10, borderWidth: 1, borderColor: theme.border }}
                        onPress={() => setSiteSwitcherVisible(true)}
                        activeOpacity={0.8}
                    >
                        <Text style={[styles.headerTitle, { color: theme.text, fontSize: 14 }]} numberOfLines={1}>
                            {activeSite?.name || 'فروشگاه من'}
                        </Text>
                        <Feather name="chevron-down" size={16} color={theme.textMuted} style={{ marginRight: 6 }} />
                    </TouchableOpacity>
                    <Text style={[styles.headerSubtitle, { color: theme.textMuted, marginTop: 4 }]} numberOfLines={1}>
                        {activeSite?.url ? activeSite.url.replace(/^https?:\/\//, '').replace(/\/$/, '') : 'نسخه پیشمو پرو 👑'}
                    </Text>
                </View>
                ...
                <View style={{ flexDirection: 'row-reverse' }}>
                    ...
                    <TouchableOpacity style={[styles.iconBtn, { backgroundColor: theme.input, borderColor: theme.border, borderWidth: 1, marginLeft: 8 }]} onPress={() => setSupportModalVisible(true)}>
            ...
            {/* 🌟 پاپ‌آپ پشتیبانی با چیدمان یک سطر و آیکون‌های باریک */}
            <Modal visible={supportModalVisible} transparent animationType="fade" onRequestClose={() => setSupportModalVisible(false)}>
                <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setSupportModalVisible(false)}>
                    <View style={styles.supportSheet}>
                        ...
                        <Text style={styles.supportTitle}>ارتباط با تیم پشتیبانی</Text>
                        
                        <View style={styles.supportGrid}>
                            <TouchableOpacity style={[styles.supportCard, { borderColor: '#10b981', backgroundColor: '#ecfdf5' }]} onPress={() => openSupportLink('https://ble.ir/09981212531')}>
                                <Feather name="message-circle" size={24} color="#10b981" />
                                <Text style={[styles.supportTxt, { color: '#047857' }]}>بله</Text>
                            </TouchableOpacity>
                            
                            <TouchableOpacity style={[styles.supportCard, { borderColor: '#0ea5e9', backgroundColor: '#f0f9ff' }]} onPress={() => openSupportLink('https://t.me/09981212531')}>
                                <FontAwesome5 name="telegram-plane" size={24} color="#0ea5e9" />
                                <Text style={[styles.supportTxt, { color: '#0369a1' }]}>تلگرام</Text>
                            </TouchableOpacity>
                            
                            <TouchableOpacity style={[styles.supportCard, { borderColor: '#64748b', backgroundColor: '#f8fafc' }]} onPress={() => openSupportLink('tel:09981212531')}>
                                <Feather name="phone-call" size={24} color="#475569" />
                                <Text style={[styles.supportTxt, { color: '#334155' }]}>تماس</Text>
                            </TouchableOpacity>
                        </View>
                    </View>
                </TouchableOpacity>
            ...
            {/* مودال کشویی مدیریت سایت‌ها */}
            <Modal visible={siteSwitcherVisible} transparent animationType="fade" onRequestClose={() => setSiteSwitcherVisible(false)}>
                <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setSiteSwitcherVisible(false)}>
                    <View style={[styles.bottomSheet, { backgroundColor: theme.card }]}>
                        <View style={styles.dragHandle} />
                        <Text style={[styles.sheetTitle, { color: theme.text }]}>انتخاب فروشگاه فعال</Text>
                        ...
                        {sites?.map((item: any) => {
                            ...
                            return (
                                <TouchableOpacity 
                                    key={item.id}
                                    style={[styles.siteItem, isActive && styles.siteItemActive, { borderColor: theme.border, backgroundColor: isActive ? (isDark ? 'rgba(16, 185, 129, 0.1)' : '#ecfdf5') : theme.input }]} 
                                    onPress={async () => {
                                        setSiteSwitcherVisible(false);
                                        await switchSite(item.id);
                                ...
                                >
                                    <View style={styles.siteItemIcon}>
                                        {isActive ? (
                                            ...
                                            <View style={[styles.circlePlaceholder, { borderColor: theme.textMuted }]} />
                                        )}
                                    </View>
                                    <View style={styles.siteItemDetails}>
                                        <Text style={[styles.siteItemName, { color: isActive ? '#10b981' : theme.text }]} numberOfLines={1}>
                                            {item.name || 'فروشگاه'}
                                        </Text>
                                        <Text style={[styles.siteItemUrl, { color: theme.textMuted }]} numberOfLines={1}>{item.url.replace(/^https?:\/\//, '')}</Text>
                                    </View>
                                </TouchableOpacity>
                            );
                        })}

                        <TouchableOpacity style={[styles.addNewBtn, { borderTopColor: theme.border }]} onPress={() => {
                            setSiteSwitcherVisible(false);
                            navigation.navigate('LoginScreen'); 
                        }}>
                            <Feather name="plus" size={20} color="#3b82f6" />
                            <Text style={styles.addNewTxt}>افزودن سایت جدید</Text>
                        </TouchableOpacity>
                    </View>
                </TouchableOpacity>
            </Modal>
...
const styles = StyleSheet.create({
    ...
    modalContainer: { borderRadius: 20, padding: 20, elevation: 5, margin: 20 },
    ...
    bottomSheet: { borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 20, maxHeight: '70%', paddingBottom: 30 },
    
    supportSheet: { backgroundColor: '#ffffff', borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 20, paddingBottom: 40 },
    supportTitle: { fontSize: 16, fontWeight: '900', color: '#0f172a', textAlign: 'center', marginBottom: 20 },
    
    // 🌟 استایل جدید برای در یک سطر بودن آیکون‌ها
    supportGrid: { flexDirection: 'row-reverse', justifyContent: 'space-between', gap: 10 },
    supportCard: { flex: 1, paddingVertical: 15, paddingHorizontal: 5, borderRadius: 12, alignItems: 'center', borderWidth: 1 },
    supportTxt: { marginTop: 8, fontSize: 11, fontWeight: 'bold', textAlign: 'center' },

    dragHandle: { width: 40, height: 4, backgroundColor: '#cbd5e1', borderRadius: 2, alignSelf: 'center', marginBottom: 20 },
    sheetTitle: { fontSize: 15, fontWeight: '900', textAlign: 'center', marginBottom: 20 },
    siteItem: { flexDirection: 'row-reverse', alignItems: 'center', paddingVertical: 12, paddingHorizontal: 16, borderRadius: 12, marginBottom: 10, borderWidth: 1 },
    siteItemActive: { borderColor: '#a7f3d0' },
    siteItemIcon: { marginLeft: 15 },
    circlePlaceholder: { width: 22, height: 22, borderRadius: 11, borderWidth: 2 },
    siteItemDetails: { flex: 1, alignItems: 'flex-end' },
    siteItemName: { fontSize: 14, fontWeight: 'bold', marginBottom: 4 },
    siteItemUrl: { fontSize: 11 },
    addNewBtn: { flexDirection: 'row-reverse', alignItems: 'center', justifyContent: 'center', paddingVertical: 16, marginTop: 10, borderTopWidth: 1 },
    addNewTxt: { fontSize: 14, fontWeight: '900', color: '#3b82f6', marginRight: 8 },
});
