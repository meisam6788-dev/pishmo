...
import { CustomerClubScreen } from './src/screens/CustomerClubScreen'; 
...
const ToolsMenuScreen: React.FC = () => {
    ...
    const [activeTool, setActiveTool] = useState<'customers' | 'club' | 'coupons' | 'taxonomy' | 'settings' | 'gallery' | 'posts' | 'bulk' | 'sms' | null>(null);
    ...
    return (
        <GlobalScreenWrapper defaultBgColor={theme.bg}>
            <ScrollView contentContainerStyle={[styles.toolsList, { paddingTop: 25 }]} showsVerticalScrollIndicator={false}>
                ...
                {/* لیست امکانات (حالت عادی و رایگان) */}
                ...
                <TouchableOpacity style={[styles.toolCard, { backgroundColor: theme.card, borderColor: theme.border }]} onPress={() => setActiveTool('coupons')}>
                    <Feather name="chevron-left" size={20} color={theme.textMuted} />
                    <View style={styles.toolInfo}><Text style={[styles.toolName, { color: theme.text }]}>کدهای تخفیف (کوپن‌ها)</Text><Text style={[styles.toolDesc, { color: theme.textMuted }]}>مدیریت کمپین‌های فروش، ساخت و تعیین سقف</Text></View>
...
function MainTabs() {
    ...
    return (
        <Tab.Navigator
            ...
            screenOptions={{
                ...
                tabBarStyle: {
                    position: 'absolute', marginHorizontal: 25, marginBottom: 25, height: 55,
                    backgroundColor: isDark ? 'rgba(30, 41, 59, 0.95)' : 'rgba(255, 255, 255, 0.95)',
                    borderRadius: 35, borderTopWidth: 0, elevation: 10, shadowColor: '#000',
                    shadowOffset: { width: 5, height: 10 }, shadowOpacity: 0.1, shadowRadius: 10,
                    paddingBottom: Platform.OS === 'ios' ? 25 : 20, paddingTop: 2,
                },
                tabBarLabelStyle: { fontSize: 10, fontWeight: '900', marginBottom: Platform.OS === 'ios' ? 0 : 5 }
            }}
        >
    ...
    options={{ 
        tabBarLabel: 'امکانات', 
        tabBarIcon: ({ color }: { color: string }) => <Feather name="settings" size={21} color={color} /> 
    }} 
/>    
<Tab.Screen 
    name="ProductsTab" 
    options={{ 
        tabBarLabel: 'محصولات', 
        tabBarIcon: ({ color }: { color: string }) => <Feather name="tag" size={22} color={color} /> 
    }}
>
...
<Tab.Screen 
    ...
    options={{ 
        tabBarLabel: 'سفارشات', 
        tabBarIcon: ({ color }: { color: string }) => <Feather name="inbox" size={22} color={color} /> 
    }}
>
...
<Tab.Screen 
    ...
    options={{ 
        tabBarLabel: 'پیشمو', 
        tabBarIcon: ({ color }: { color: string }) => <Feather name="activity" size={22} color={color} /> 
    }}
>
        ...
        <NavigationContainer>
            <Stack.Navigator screenOptions={{ headerShown: false }}>
                ...
                <Stack.Screen name="LoginScreen">
                    {() => <GlobalScreenWrapper defaultBgColor={isDark ? '#0f172a' : '#ffffff'}><LoginScreen /></GlobalScreenWrapper>}
                </Stack.Screen>
            </Stack.Navigator>
        </NavigationContainer>
    );
